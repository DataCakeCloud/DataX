...
public class HdfsReader extends Reader {
    ...
    public static class Job extends Reader.Job {
        ...
        public void init() {
            ...
            this.validate();
            String pathStr = this.readerOriginConfig.getString(Key.PATH);
            dfsUtil = new DFSUtil(this.readerOriginConfig);
            Boolean isCheckFile = this.readerOriginConfig.getBool(Key.IS_CHECK_FILE, false);
            if(isCheckFile && (pathStr.startsWith("oss://")||pathStr.startsWith("s3://"))){
                checkSuccessFile(pathStr,dfsUtil.getFileSystem());
            }
        ...
        public void checkSuccessFile(String pathStr, FileSystem fs) {
            try {
                Path path;
                if(fs.isFile(new Path(pathStr))){
                    path = new Path(pathStr);
                }else{
                    path = new Path(pathStr + "/_SUCCESS");
                }
                try {
                    for(int i=0;i<12;i++){
                        LOG.info(String.format("检测文件名称[%s]",path));
                        boolean exists = fs.exists(path);
                        if (exists){
                            LOG.info(String.format("[%s]文件已存在",path));
                            return;
                        }
                        Log.info(String.format("文件检查第%s次........",i));
                        Thread.sleep(300000L);
                    }
                }catch (Exception e){
                    System.out.println("检测文件进程异常！！！");
                }
            }catch (IOException e){
                throw DataXException.asDataXException(HdfsReaderErrorCode.FILE_CHECK_FAILED_ERROR,
                        String.format("[%s]文件检查失败，请先教研路径是否存在",path));
            }
            throw DataXException.asDataXException(HdfsReaderErrorCode.FILE_CHECK_FAILED_ERROR,
                    String.format("[%s]文件检查失败，请先教研路径是否存在",path));
        }
        ...
        public void validate(){
            ...
            if( !specifiedFileType.equalsIgnoreCase(Constant.ORC) &&
                ...
                String message = "HdfsReader插件目前支持ORC, TEXT, CSV, SEQUENCE, PARQUET,RC,六种格式的文件," +
        ...
        public void prepare() {
            ...
            try{
                boolean exists = dfsUtil.getFileSystem().exists(new Path(path.get(0)));
                if(!exists){
                    Boolean nullPointExit = this.readerOriginConfig.getBool(Key.NULL_POINT_EXIT, false);
                    if (nullPointExit){
                        LOG.warn("数据为空 任务正常终止");
                        System.exit(0);
                    }
                }
            }catch (IOException e){
                throw DataXException.asDataXException(HdfsReaderErrorCode.IO_EXCEPTION,
                        String.format("检验文件是否存在失败，请检查网络是否正常 %s",e.getMessage()));
            }
