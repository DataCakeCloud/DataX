...
public class Engine {
    ...
    public static Configuration filterSensitiveConfiguration(Configuration configuration){
        ...
        for (final String key : keys) {
            ...
            if(StringUtils.containsIgnoreCase(key, "fs.s3a.access.key")){
                String hadoopKey = key.replace( ".fs.s3a.access.key","");
                String hadoopConfStr = configuration.getString(hadoopKey);
                Map<String,Object> map = JSON.parseObject(hadoopConfStr, Map.class);
                map.put("fs.s3a.access.key","*****************");
                map.put("fs.s3a.secret.key","*****************");
                configuration.set(hadoopKey,map);
            }
...
//        System.setProperty("java.security.krb5.kdc", "172.17.53.122");
        int exitCode = 0;
        try {
            Engine.entry(args);
        } catch (Throwable e) {
