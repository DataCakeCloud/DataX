...
public class DFSUtil {
    ...
    private HashSet<String> getHDFSAllFilesNORegex(String path, FileSystem hdfs) throws IOException {
        ...
        for (FileStatus f : stats) {
            ...
            if (f.isDirectory()) {
                ...
                if(!f.getPath().getName().endsWith("_SUCCESS")){
                    addSourceFileByType(f.getPath().toString());
                }
            } else {
    ...
    public void parquetFileStartRead(String sourceParquetFilePath, Configuration readerSliceConfig,
        ...
        try {
            ...
            ParquetReader<Group> build= reader.withConf(this.hadoopConf).build();
    ...
    //                    else {
                        try{
                            if(line.getType().getType(i).asPrimitiveType().getPrimitiveTypeName() ==
                                    PrimitiveType.PrimitiveTypeName.FIXED_LEN_BYTE_ARRAY){
                                GroupType types = line.getType();
                                org.apache.parquet.schema.Type type = types.getType(i);
                                ...
                                BigDecimal decimalValue = binaryToDecimal(precision, scale, line.getBinary(i, 0).getBytes());
                                ...
                                String value = decimalFormat.format(decimalValue);
                                recordFields.add(value.toString());
                            } else if(line.getType().getType(i).asPrimitiveType().getPrimitiveTypeName() ==
                            ...
                            }else if(line.getType().getType(i).asPrimitiveType().getPrimitiveTypeName() == PrimitiveType.PrimitiveTypeName.INT64
                                    && line.getType().getType(i).getOriginalType() != null
                                    && line.getType().getType(i).getOriginalType().name().equals(OriginalType.DECIMAL.name())){
                                GroupType types = line.getType();
                                org.apache.parquet.schema.Type type = types.getType(i);
                                long decimalValue = line.getLong(i, 0);
                                int precision = type.asPrimitiveType().getDecimalMetadata().getPrecision();
                                int scale = type.asPrimitiveType().getDecimalMetadata().getScale();
                                double doubleValue = convertDecimalToDouble(decimalValue, precision, scale);
                                recordFields.add(doubleValue);
                            } else{
                            ...
                            GroupType types = line.getType();
                            org.apache.parquet.schema.Type type = types.getType(i);
                            String name = type.asPrimitiveType().getPrimitiveTypeName().name();
                            if("INT64".equalsIgnoreCase(name) || "INT32".equalsIgnoreCase(name) || "INT96".equalsIgnoreCase(name)){
                                recordFields.add(0);
                            } else if ("DOUBLE".equalsIgnoreCase(name) || "FLOAT".equalsIgnoreCase(name)) {
                                recordFields.add(0.0);
...
//            System.out.println(result);
    ...
    public static double convertDecimalToDouble(long decimalValue, int precision, int scale) {
        BigDecimal decimal = BigDecimal.valueOf(decimalValue);
        BigDecimal scaledDecimal = decimal.scaleByPowerOfTen(-scale);
