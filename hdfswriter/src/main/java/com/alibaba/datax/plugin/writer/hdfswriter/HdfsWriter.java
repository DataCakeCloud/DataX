...
public class HdfsWriter extends Writer {
    public static class Job extends Writer.Job {
        ...
        private void validateParameter() {
            ...
            if(!path.startsWith("/") && !path.startsWith("s3://") && !path.startsWith("obs://")){
...
//            createTableByGateWay();
            ...
            if(hdfsHelper.isPathexists(path)){
                ...
                try {
                    ...
                    throw DataXException.asDataXException(HdfsWriterErrorCode.CONNECT_HDFS_IO_ERROR,
                            String.format("路径创建失败:%s", path));
                }
            }
        }
        ...
        public void post() {
            ...
            String successFile;
            if (path.endsWith("/")){
                successFile = String.format("%s_SUCCESS",path);
            }else{
                successFile = String.format("%s/_SUCCESS",path);
            }
            LOG.info(String.format("生成success文件:[%s]",successFile));
            try {
                hdfsHelper.fileSystem.createNewFile(new Path(successFile));
            }catch (IOException e){
                LOG.error(e.getMessage());
                throw DataXException.asDataXException(HdfsWriterErrorCode.CONNECT_HDFS_IO_ERROR,
                        String.format("路径创建失败:%s", path));
